...
namespace Destrospean.zoeoeBorrowed
{
    public class LODData
    {
        ...
        {
            var resource = (GenericRCOLResource)Resource;
            var chunkReferenceMap = new Dictionary<MeshGroupData, Dictionary<ChunkReferences, TGIBlock>>();
            var mtstEntryMap = new Dictionary<MeshGroupData, Dictionary<MTST.Entry, TGIBlock>>();
            foreach (var meshGroup in MeshGroups)
            {
                chunkReferenceMap[meshGroup] = new Dictionary<ChunkReferences, TGIBlock>();
                foreach (ChunkReferences chunkReference in System.Enum.GetValues(typeof(ChunkReferences)))
                {
                    chunkReferenceMap[meshGroup][chunkReference] = resource.ChunkEntries[((GenericRCOLResource.ChunkReference)meshGroup.MeshGroup.GetType().GetProperty(chunkReference.ToString()).GetValue(meshGroup.MeshGroup, null)).TGIBlockIndex + resource.PublicChunks].TGIBlock;
                }
                if (meshGroup.DirectMATD == null)
                {
                    mtstEntryMap[meshGroup] = new Dictionary<MTST.Entry, TGIBlock>();
                    foreach (var entry in meshGroup.MaterialSet.Entries)
                    {
                        mtstEntryMap[meshGroup][entry] = resource.ChunkEntries[entry.Index.TGIBlockIndex + resource.PublicChunks].TGIBlock;
                    }
                }
            }
            ...
            {
                var meshGroup = MeshGroups[groupIndex];
                foreach (var tgiBlock in mtstEntryMap[meshGroup].Values)
                {
            ...
            foreach (var meshGroup in MeshGroups)
            {
                foreach (ChunkReferences chunkReference in System.Enum.GetValues(typeof(ChunkReferences)))
                {
                    ((GenericRCOLResource.ChunkReference)meshGroup.MeshGroup.GetType().GetProperty(chunkReference.ToString()).GetValue(meshGroup.MeshGroup, null)).TGIBlockIndex = resource.ChunkEntries.FindIndex(x => x.TGIBlock.Equals(chunkReferenceMap[meshGroup][chunkReference])) - resource.PublicChunks;
                }
                if (meshGroup.DirectMATD == null)
                {
                    ...
                    {
                        entry.Index.TGIBlockIndex = resource.ChunkEntries.FindIndex(x => x.TGIBlock.Equals(mtstEntryMap[meshGroup][entry])) - resource.PublicChunks;
                    }
                }
            }
        }
