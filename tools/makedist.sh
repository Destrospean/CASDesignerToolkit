...
docker run --rm --name casdesignertoolkit-build -v "$(pwd)/..":/CASDesignerToolkit casdesignertoolkit bash -c "/CASDesignerToolkit/tools/_build.sh && cd /CASDesignerToolkit/CASDesignerToolkit/bin/Release/ && rm *.mdb *.pdb *.xml && cp /CASDesignerToolkit/GameFolders.xml ."
