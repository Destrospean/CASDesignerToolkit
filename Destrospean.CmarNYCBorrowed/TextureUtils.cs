...
namespace Destrospean.CmarNYCBorrowed
{
    public static class TextureUtils
    {
        ...
        /*
        public Bitmap GetSkintone(TONE tone, AgeGender age, AgeGender gender, PartTypeFlags partType, Bitmap colorRamp, float colorSlider, float cutnessSlider, float cleavageSlider)
        {
            if (tone == null)
            {
                return null;
            ...
            TONE.SkinSet skinSet = tone.GetSkinSet(Species.Human, age, gender, partType);
            Bitmap details = FetchGameImageFromDDS(skinSet.LightSkinLink, ref errorList);
            using (var graphics = Graphics.FromImage(details))
            {
                Bitmap dark = FetchGameImageFromDDS(skinSet.darkLink, ref errorList);
                alphaMatrix[3][3] = colorSlider;
                var convert = new ColorMatrix(alphaMatrix);
                var attributes = new ImageAttributes();
                attributes.SetColorMatrix(convert, ColorMatrixFlag.Default, ColorAdjustType.Bitmap);
                graphics.DrawImage(dark, new Rectangle(0, 0, details.Width, details.Height), 0, 0, dark.Width, dark.Height, GraphicsUnit.Pixel, attributes);
                dark.Dispose();
                Bitmap overlay = FetchGameImageFromDDS(skinSet.overlayLink, ref errorList);
                if (overlay != null)
                {
            ...
            {
                ptr = bitmapData.Scan0 + bitmapData.Stride * (details.Height - 1);
            }
            var bytes = Math.Abs(bitmapData.Stride) * details.Height;
            var detail = new byte[bytes];
            System.Runtime.InteropServices.Marshal.Copy(ptr, detail, 0, bytes);
            // argbValues1[i] = blue
            ...
            for (var i = 0; i < detail.Length; i += 4)
            {
                ...
                int col,
                row = Math.DivRem(i / 4, details.Width, out col);
                if (row > 665 && row < 842 && col < 85)
                {
                    detail[i + 3] = 0;
                }
            }
            System.Runtime.InteropServices.Marshal.Copy(detail, 0, ptr, bytes);
            details.UnlockBits(bitmapData);
            if (partType == PartTypeFlags.Face)
            {
                Image faceOver = Properties.Resources.amFaceOverlay;
                using (var graphics = Graphics.FromImage(details))
                {
                    graphics.DrawImage(faceOver, 0, 0, details.Width, details.Height);
                }
            }
            return details;
        }
